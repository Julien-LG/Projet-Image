...
void plusProcheVoisinTest() {
    ASSERT( plusProcheVoisin( Point( {1,0,1}),
        ...
        == 1);
	/*cout << "resultat du test du plus proche voisin : " << plusProcheVoisin(Point({ 1,0,1 }), EnsemblePoints({ Point({2,3,6}), Point({2,6,9}), Point({1,0,0}), Point({-1,2,0}) })) << endl << endl;
	cout << "Test avec distanceAEnsemble : " << distanceAEnsemble(Point({ 1,0,1 }), EnsemblePoints({ Point({2,3,6}), Point({1,0,0}), Point({-1,2,0}) })) << endl << endl;*/
}
...
void KMoyenneTest(){
	...
	cout << "nb d'ensemble de points" << P.size() << endl;
	cout << " nb de points par ensemble" << P[0].size() << endl;
	cout << endl << endl;
	...
	cout << endl;
	cout << "test de barycentre" << endl;
	for (int i = 0; i < barycentre(P).size(); i++)
	{
		cout << barycentre(P)[i] << " ";
	}
	cout << endl;
